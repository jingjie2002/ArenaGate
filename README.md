...
- 运营通知：鉴权后可推送 `server_notice`，可从 GameOps 动态读取公告。
- 维护态：可通过环境变量或 GameOps 开启 `maintenance_state`，并拒绝新的匹配入场。
- 封禁状态：可从 GameOps 读取玩家封禁状态，封禁玩家无法进入匹配。
...
internal/opsclient/   GameOps 运营状态客户端
